...
/**
 * HappitReporter constructor
 */

function HappitReporter(baseReporterDecorator, formatError, config) {
  var self = this;
  ...
  if (config && config.happitReporter) {
    // merge defaults
    Object.keys(self.options).forEach(function(optionName){
      if (config.happitReporter.hasOwnProperty(optionName)) {
        self.options[optionName] = config.happitReporter[optionName];
      }
    });
  }
...
HappitReporter.prototype.reset = function() {
...
HappitReporter.prototype.onRunStart = function (browsers) {
...
HappitReporter.prototype.onBrowserLog = function(browser, log) {
...
HappitReporter.prototype.onSpecComplete = function(browser, result) {
...
HappitReporter.prototype.onRunComplete = function() {
  this.draw();
...
HappitReporter.prototype.onBrowserStart = function (browser) {
  this._browsers.push(browser);
...
HappitReporter.prototype.onBrowserError = function(browser, error) {
  this.browserErrors.push({'browser': browser, 'error': error});
};

exports.HappitReporter = HappitReporter;
